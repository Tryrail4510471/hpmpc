...
阶段五已完成真实 `seq=16` 小样本端到端 CPU 推理。使用真实 TinyBERT encoder 权重、真实 embedding 权重和 tokenizer input，CPU online `getTime` 约 `6.30s`，通信量约 P0 发送 `7.099MB`、P1/P2 发送 `5.201MB`。CUDA 版本可完成但仍有 CUTLASS internal error 日志，暂不作为可信性能结论。
