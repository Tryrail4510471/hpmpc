...
11. Stage 18 已扫描 LayerNorm rsqrt 初值；20 轮最佳边界约为 `init=1/24`，但 final drift mean 仍约 `0.0248`，且收敛区间过窄，不进入默认路径。
...
2. 设计结构性 LayerNorm rsqrt 优化，例如分段/查表 rsqrt，或先估计 variance range 再选择保守初值。
...
- LayerNorm 20 轮配更大初值的最佳实验点是 `init=1/24`，final drift mean 约 `0.0248`；它可作为研究边界，不作为默认优化。
